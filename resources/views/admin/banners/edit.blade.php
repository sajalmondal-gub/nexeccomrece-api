...
@section('title', 'Edit Banner - NexCommerce')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="rounded-3xl border border-indigo-950/40 bg-slate-900/60 p-8 backdrop-blur-xl shadow-xl shadow-purple-950/5">
        <form method="POST" action="{{ route('admin.banners.update', $banner->id) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            @if($banner->image)
            <div class="mb-6">
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Current Image</label>
                <div class="h-40 w-full rounded-xl bg-slate-950 flex items-center justify-center border border-indigo-950/40 overflow-hidden shadow-lg">
            ...
            <!-- Image Upload -->
            <div>
                <label for="image" class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Replace Image (Optional)</label>
                ...
                <p class="mt-2 text-[10px] text-slate-500">Leave blank to keep the current image.</p>
            ...
            <div>
                ...
                <input type="text" name="link" id="link" value="{{ old('link', $banner->link) }}" placeholder="https://app.nexcommerce.com/sale"
                    class="block w-full rounded-xl border border-indigo-950/60 bg-slate-950 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500 transition-all">
